$(function(){
	...
	$('#search').die().live('click',function(){
		...
		jQuery("#grid-table").jqGrid('setGridParam', {
		    url : common.getContextPath()+'/report/getWeekly2Page.do',
			postData:{
                "starttime":starttime,
                "endtime":endtime
			},
    ...
    $('#export').die().live('click',function(){
        ...
        window.open(common.getContextPath()+
            '/report/d_weekly2.do?starttime='+starttime+ '&endtime='+endtime);
    });
});
...
function tabSHow(){
    var starttime = $('#starttime').val();
    var endtime = $('#endtime').val();
	var colNames = [ '姓名', '任务描述','开始时间','结束时间','审核人','完成情况','任务ID'];
	var colModel = [ 
		             {name : 'v1',index : 'v1',width:50},
                     {name : 'v2',index : 'v2'},
                     {name : 'v3',index : 'v3',width:70},
                     {name : 'v4',index : 'v4',width:70},
        			 {name : 'v5',index : 'v5',width:50},
		             {name : 'v6',index : 'v6',width:50},
