$(function(){
    ...
    $('.view').die().live('click',function(){
        ...
        top.layui.use('layer', function(){
            ...
            layer.open({
                ...
                content: common.getContextPath()+'/pa/faultForm.do?type=view&id='+id
            });
        });
    });
    $('#new').die().live('click',function(){
        ...
        top.layui.use('layer', function(){
            ...
            layer.open({
                ...
                shadeClose : true,
                area : [ dialogWith+'px', dialogHeight+'px' ],
                content: common.getContextPath()+'/pa/faultForm.do?type=new',
                end: function(){
                    refreshTable();
                },
                btn: ['提交', '取消'],
        ...
        for ( var i = 0; i < ids.length; i++) {
            ...
            top.layui.use('layer', function(){
                ...
                layer.open({
                    ...
                    shadeClose : true,
                    area : [ dialogWith+'px', dialogHeight+'px' ],
                    content: common.getContextPath()+'/pa/faultForm.do?type=edit&id='+id,
                    end: function(){
                        refreshTable();
                    },
                    btn: ['提交', '取消'],
	...
	$('#search').die().live('click',function(){
		...
		jQuery("#grid-table").jqGrid('setGridParam', {
		    url : common.getContextPath()+'/pa/getFaultPage.do',
			postData:{
                "starttime":starttime,
                "endtime":endtime
			},
...
function deleteRecord(id){
    top.layui.use('layer', function(){
        ...
        layer.confirm('确认禁用这笔记录?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                url: common.getContextPath()+'/pa/deleteFault.do',
                type: 'Post',
                dataType: "json",
                data:{
                    id:id
