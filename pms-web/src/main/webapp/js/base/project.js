$(function(){
	...
	$('#new').die().live('click',function(){
		top.layui.use('layer', function() {
			...
			layer.open({
				...
				shadeClose : true,
				area : [ '800px', '400px' ],
				content : common.getContextPath()+'/base/projectForm.do?type=new',
				end : function() {
					refreshTable();
				},
				btn : [ '提交', '取消' ],
	...
	$('#search').die().live('click',function(){
		...
		jQuery("#grid-table").jqGrid('setGridParam', {
		    url : common.getContextPath()+'/base/getProjectPage.do',
			postData:{
				"name":name,
				"status":status
			},
		...
		for ( var i = 0; i < ids.length; i++) {
            ...
            top.layui.use('layer', function(){
                ...
                layer.open({
    				...
    				shadeClose : true,
    				area : [ '900px', '400px'  ],
                	content: common.getContextPath()+'/base/projectForm.do?type=edit&id='+id,
                	end: function(){ 
                		refreshTable();
    				},  
                	btn: ['提交', '取消'],
...
function tabSHow(){
    var name=$('#name').val();
	var status=$('input[name=status]:checked').val();
	var colNames = [ '项目名称', '状态','排序','创建时间','最近修改时间'];
	var colModel = [
...
function deleteRecord(id){
	top.layui.use('layer', function(){
		  ...
		  layer.confirm('确认禁用这笔记录?', {icon: 3, title:'提示'}, function(index){
			  $.ajax({
			        url: common.getContextPath()+'/base/deleteProject.do',
			        type: 'Post',
			        dataType: "json",
			        data:{
			        	id:id
