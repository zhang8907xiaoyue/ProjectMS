				...
				<c:when test="${type=='new'}">
					<form id="newForm" class="form-horizontal" role="form" style="padding: 20px">
						<div class="form-group">
							<label class="col-xs-1 control-label no-padding-right" for="year">
								<font style="color:red">*</font>年份</label>
							<div class="col-xs-2">
								<select id="year" class="col-xs-10">
									<%
							...
							<label class="col-xs-1 control-label no-padding-right" for="month">
								<font style="color:red">*</font>月份</label>
							<div class="col-xs-2">
								<select id="month" class="col-xs-10">
									<%
										...
										for(int i=1;i<=12;i++){
										    ...
										    if(month==i){
										        %>
													<option selected="selected" value="<%=monthStr%>"><%=monthStr%></option>
												<%
											}else{
												%>
						...
						<div class="form-group">
							...
							<div class="col-xs-2">
								<select id="year" class="col-xs-10">
									<%
										int year=Integer.parseInt(request.getAttribute("year").toString());
										String yearTemp=request.getAttribute("month_year").toString();
										for(int i=0;i<4;i++){
										    if(yearTemp.equals(String.valueOf(year-i))){
										        %>
													<option value="<%=yearTemp%>" selected="selected"><%=yearTemp%></option>
												<%
											}else{
												%>
							...
							<div class="col-xs-2">
								<select id="month" class="col-xs-10">
									<%
										String monthStr="";
										String monthTemp=request.getAttribute("month_month").toString();
										for(int i=1;i<=12;i++){
													if(i<10){
														monthStr=String.format("%02d", i);
													}else{
													...
													if(monthTemp.equals(String.valueOf(monthStr))){
														%>
															<option value="<%=monthTemp%>" selected="selected"><%=monthTemp%></option>
														<%
													}else{
														%>
															<option value="<%=monthStr%>"><%=monthStr%></option>
							...
							<div class="col-xs-2">
								...
								<input id="input-employee" type="text" value="${monthbest.employeeid}" style="display: none">
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-1 control-label no-padding-right" for="title">
								<font style="color:red">*</font>标题</label>
							<div class="col-xs-10">
								<input type="text" id="title" class="col-xs-10" value="${monthbest.title}"/>
							</div>
						</div>
						<div class="form-group">
							<label class="col-xs-1 control-label no-padding-right" for="content">
								需求描述</label>
							<div class="col-xs-10">
								<textarea id="content" name="textarea" cols="100" rows="8" style="width:700px;height:200px;visibility:hidden;">${monthbest.contenthtml}</textarea>

							</div>
						</div>

						<input type="text" id="form-id" style="display: none" value="${monthbest.id}">
					</form>
				</c:when>
				<c:otherwise>
					<div class="zxy-title">
						<span class="storyid">${monthbest.id}</span>
						<strong>${monthbest.title}</strong>
					</div>
					<div class="zxy-body">
						<div class="col-xs-10">
							<div class="zxy-content">
								...
								<table>
									<tr>
										<td class="tb-name">月份:</td>
										<td class="tb-value">${monthbest.month}</td>
										<td class="tb-name">姓名:</td>
										<td class="tb-value">${monthbest.employee}</td>
									</tr>
								</table>
								</div>
									${monthbest.contenthtml}
							</div>
						</div>
					</div>
				</c:otherwise>
...
<script type="text/javascript"
		src="<%=request.getContextPath()%>/js/pa/monthbestForm.js?v=<%=Constant.VERSION%>"></script>
